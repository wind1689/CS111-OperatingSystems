...
do_fork(process_t *parent)
{
	...
	int child_pid;
	printf("Hello world~\n");
	for (child_pid = 2; child_pid < NPROCS; ++child_pid)
	{
		//When the process state is empty, use it to create a child process
		if (proc_array[child_pid].p_state != P_EMPTY)
		{
			process_t *child;
			child = &proc_array[child_pid];
			child->p_pid = child_pid;
			child->p_state = P_RUNNABLE;
			child->p_registers.reg_esp = PROC1_STACK_ADDR + child_pid * PROC_STACK_SIZE;
...
copy_stack(process_t *dest, process_t *src)
{
	...
	dest_stack_top = dest->p_registers.reg_esp; /* YOUR CODE HERE */;
	...
	memcpy(dest, src, (src_stack_top - src_stack_bottom + 1));
