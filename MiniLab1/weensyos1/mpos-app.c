...
start(void)
{
	...
	if (p == 0)
		...
		do {
			status = sys_wait(p);    //Now, p is the child process ID
		...
		if (checker != 0) {
			...
			sys_exit(0);  //0 represents success in running process or function;
...
run_child(void)
{
	...
	sys_exit(1000);  //When the child process exit, the status = 1000;
}
