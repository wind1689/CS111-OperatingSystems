...
int main(int argc, char *argv[])
{
 ...
 flag:
	...
	if (notification_type) {
		...
		my_data[0] = Bx;
		my_data[1] = By;
		if (ioctl(devfd, OSPRDIOCNOTIFICATION, my_data) == -1) {
			perror("ioctl OSPRDIOCNOTIFICATION");
			exit(1);
		}
	}
	...
	else if (mode & O_WRONLY)
	{
		...
		my_data2 = malloc (sizeof(int) * 2);
		my_data2[0] = offset;
		my_data2[1] = (size == -1) ? string_size : size;
		if (ioctl(devfd, OSPRDIOCWAKENOTIFICATION, my_data2) == -1) {
			perror("ioctl OSPRDIOCWAKENOTIFICATION");
			exit(1);
		}
